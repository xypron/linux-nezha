...
static void mbox_intc_tx_msg(struct irq_data *d, uint8_t request)
{
	...
	if (mbox_send_message(intc->chan, &msg) >= 0)
		mbox_client_txdone(intc->chan, 0);
}
...
static int mbox_intc_probe(struct platform_device *pdev)
{
	...
	intc->chan = mbox_request_channel(&intc->cl, 0);
	if (IS_ERR(intc->chan)) {
		dev_err(dev, "Failed to request mailbox channel\n");
		return PTR_ERR(intc->chan);
	}
	...
	if (IS_ERR(intc->domain)) {
		...
		mbox_free_channel(intc->chan);
		return PTR_ERR(intc->domain);
	}
